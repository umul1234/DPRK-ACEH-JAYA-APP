...
.section-kicker { color: var(--primary); font-size: 11px; font-weight: 800; letter-spacing: 1.2px; text-transform: uppercase; margin-bottom: 6px; }
...
.service-box:hover { transform: translateY(-6px); border-top-color: var(--primary); box-shadow: var(--shadow-lg); }
.service-icon {
...
.footer-brand { display: flex; align-items: flex-start; gap: 16px; margin-bottom: 24px; }
.footer-logo {
...
.footer-brand-name {
    ...
    font-size: 18px; font-weight: 800; letter-spacing: 0.5px; margin-bottom: 4px;
}
.footer-brand-subtitle {
    color: var(--gold); font-size: 11px; font-weight: 600;
    letter-spacing: 1.2px; text-transform: uppercase;
}
.footer-description { line-height: 1.8; color: #94a3b8; margin-bottom: 28px; max-width: 320px; }
...
.footer-social-item {
    ...
    justify-content: center; color: #cbd5e1; font-size: 16px; transition: all 0.3s ease; cursor: pointer;
}
.footer-social-item:hover { background: var(--gold); color: #041c17; border-color: var(--gold); transform: translateY(-3px); }
.footer-heading {
    color: #ffffff; font-family: 'Plus Jakarta Sans', sans-serif;
    font-size: 15px; font-weight: 700; margin-bottom: 24px; letter-spacing: 0.5px;
}
.footer-link {
    color: #94a3b8; text-decoration: none; display: block; margin-bottom: 14px;
    transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); position: relative;
}
.footer-link:hover { color: var(--gold); transform: translateX(6px); }
.footer-contact-item { display: flex; gap: 12px; margin-bottom: 16px; line-height: 1.6; align-items: flex-start; }
.footer-contact-icon { color: var(--gold); font-size: 16px; margin-top: 2px; flex-shrink: 0; }
.footer-divider {
    height: 1px; background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.1), transparent); margin-bottom: 32px;
}
.footer-bottom {
    display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; font-size: 12px; color: #64748b;
}
.footer-bottom-right { display: flex; align-items: center; gap: 8px; }
.footer-bottom-right::before {
    content: ''; width: 6px; height: 6px; background: var(--primary); border-radius: 50%; display: inline-block;
}
...
@media (max-width: 900px) {
    ...
    .content, .hero-content, .info-inner { width: 92%; }
...
@media (max-width: 600px) {
    .premium-footer { padding: 48px 6% 24px; }
...
if st.session_state.page == "Beranda":
    ...
    for i, (icon, title, desc, target) in enumerate(services):
        with service_cols[i]:
            st.markdown(f'<div class="service-box"><div class="service-icon">{icon}</div><div class="service-title">{title}</div><div class="service-desc">{desc}</div></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="info-strip"><div class="info-inner">', unsafe_allow_html=True)
    stats = [("2024–2029", "Masa Jabatan"), ("3", "Pimpinan DPRK"), ("5", "Komisi / Alat Kelengkapan"), ("24/7", "Akses Informasi")]
    stat_cols = st.columns(4)
    for i, (number, label) in enumerate(stats):
        with stat_cols[i]:
            st.markdown(f'<div class="info-item"><div class="info-number">{number}</div><div class="info-label">{label}</div></div>', unsafe_allow_html=True)
    st.markdown('</div></div><div class="content" id="berita">', unsafe_allow_html=True)
    ...
    with col_news:
        main_news = NEWS[0]
        st.markdown(f'<div class="news-main"><img class="news-main-img" src="{main_news["image"]}"><div class="news-main-body"><div class="news-tag">{main_news["tag"]}</div><h3>{main_news["title"]}</h3><p style="color:var(--text-muted);font-size:13px;line-height:1.65;margin-bottom:12px;">{main_news["desc"]}</p><div class="news-date">🕒 {main_news["date"]}</div></div></div>', unsafe_allow_html=True)
        for item in NEWS[1:]:
            st.markdown(f'<div class="news-side"><img class="news-side-img" src="{item["image"]}"><div><div class="news-tag">{item["tag"]}</div><h4>{item["title"]}</h4><p>{item["desc"]}</p><div class="news-date" style="margin-top:8px;">🕒 {item["date"]}</div></div></div>', unsafe_allow_html=True)

    with col_agenda:
        st.markdown('<div class="agenda-wrap"><div class="section-kicker">Jadwal</div><div style="font-family:\'Plus Jakarta Sans\';font-size:19px;font-weight:800;color:#0f172a;margin-bottom:16px;">Agenda Terdekat</div>', unsafe_allow_html=True)
        for day, month, title, desc in AGENDA:
            st.markdown(f'<div class="agenda-row"><div class="agenda-date"><strong>{day}</strong><span>{month}</span></div><div><div class="agenda-title">{title}</div><div class="agenda-desc">{desc}</div></div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    st.markdown("</section></div>", unsafe_allow_html=True)
...
# =========================================================
    ...
    st.markdown('<section class="section"><div class="section-kicker">Tentang DPRK</div><h2 class="section-title">Profil DPRK Aceh Jaya</h2><div class="section-desc">Dewan Perwakilan Rakyat Kabupaten Aceh Jaya sebagai unsur penyelenggara pemerintahan daerah bersama pemerintah daerah menjalankan fungsi legislasi, anggaran, dan pengawasan sesuai ketentuan peraturan perundang-undangan.</div></section>', unsafe_allow_html=True)
    st.markdown('<div style="background:var(--white);border:1px solid var(--border);border-radius:12px;padding:32px;margin-bottom:40px;"><div class="section-kicker">Pimpinan</div><h2 class="section-title" style="font-size:22px;">Pimpinan DPRK Aceh Jaya</h2><div class="section-desc">Masa Jabatan 2024–2029</div></div>', unsafe_allow_html=True)
    
    cols = st.columns(3)
    for i, (role, name, desc) in enumerate(PIMPINAN):
        with cols[i]:
            initials = "".join([word[0] for word in name.split()[:2]])
            st.markdown(f'<div class="profile-card"><div class="profile-photo">{initials}</div><div class="profile-role">{role}</div><div class="profile-name">{name}</div><div class="profile-desc">{desc}</div></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
# =========================================================
    ...
    st.markdown('<div class="content">', unsafe_allow_html=True)
    st.markdown('<section class="section"><div class="section-kicker">Publikasi</div><h2 class="section-title">Berita & Agenda DPRK</h2><div class="section-desc">Informasi kegiatan, rapat, agenda, dan aktivitas DPRK Aceh Jaya.</div></section>', unsafe_allow_html=True)
    
    for item in NEWS:
        st.markdown(f'<div style="background:var(--white);border:1px solid var(--border);border-radius:12px;padding:20px;margin-bottom:20px;display:flex;gap:24px;transition:all 0.2s;"><img src="{item["image"]}" style="width:280px;height:170px;object-fit:cover;border-radius:8px;flex-shrink:0;"><div style="padding:4px 0;flex:1;"><div class="news-tag">{item["tag"]}</div><h3 style="color:#0f172a;font-family:\'Plus Jakarta Sans\';font-size:20px;margin:0 0 10px;font-weight:700;">{item["title"]}</h3><p style="color:var(--text-muted);font-size:13px;line-height:1.7;margin-bottom:12px;">{item["desc"]}</p><div class="news-date">🕒 {item["date"]}</div></div></div>', unsafe_allow_html=True)

    st.markdown('<div style="margin-top:56px;" class="section-kicker">Agenda</div><h2 class="section-title" style="font-size:23px;margin-bottom:20px;">Agenda Terdekat</h2>', unsafe_allow_html=True)
    agenda_cols = st.columns(3)
    for i, (day, month, title, desc) in enumerate(AGENDA):
        with agenda_cols[i]:
            st.markdown(f'<div class="agenda-wrap" style="height:100%;"><div class="agenda-date"><strong>{day}</strong><span>{month}</span></div><div style="margin-top:16px;" class="agenda-title">{title}</div><div class="agenda-desc">{desc}</div></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
# =========================================================
    ...
    st.markdown('<div class="content">', unsafe_allow_html=True)
    st.markdown('<section class="section"><div class="section-kicker">Pelayanan Masyarakat</div><h2 class="section-title">Layanan Aspirasi & Pengaduan</h2><div class="section-desc">Sampaikan aspirasi, laporan, atau pengaduan kepada DPRK Aceh Jaya.</div></section>', unsafe_allow_html=True)
    
    st.markdown('<div style="background:#fffbeb;border-left:4px solid #f59e0b;padding:16px 20px;border-radius:8px;margin-bottom:32px;display:flex;gap:12px;align-items:flex-start;"><span style="font-size:20px;">🚨</span><div><strong style="color:#92400e;font-size:14px;">Layanan Tanggap Darurat Banjir</strong><p style="color:#92400e;font-size:13px;margin:4px 0 0;line-height:1.5;">Prioritas pengaduan dampak bencana akan diproses dalam 1x24 jam. Silakan pilih kategori "Bencana / Banjir" pada formulir di bawah.</p></div></div>', unsafe_allow_html=True)

    col1, col2 = st.columns([1.8, 1])
    with col1:
        st.markdown('<div class="section-kicker">Formulir</div><h3 style="color:#0f172a;font-family:\'Plus Jakarta Sans\';font-size:20px;margin-bottom:16px;font-weight:700;">Sampaikan Aspirasi Anda</h3>', unsafe_allow_html=True)
        with st.form("form_aduan"):
            ...
            with c2:
                kategori = st.selectbox("Kategori Pengaduan *", ["🚨 Bencana / Banjir (Prioritas Tinggi)", "🛣️ Infrastruktur & Jalan", "🏥 Pelayanan Publik", "📜 Legislasi & Qanun", "💡 Lainnya"])
    ...
    with col2:
        st.markdown('<div class="agenda-wrap"><div class="section-kicker">Kontak</div><h3 style="color:#0f172a;font-family:\'Plus Jakarta Sans\';font-size:19px;margin-top:0;font-weight:700;">Hubungi Kami</h3><div style="margin-top:20px;"><p style="font-size:12px;color:#0f172a;font-weight:700;margin:0 0 4px;">📞 Telepon</p><p style="font-size:13px;color:var(--text-muted);margin:0 0 16px;">(0655) 12345</p><p style="font-size:12px;color:#0f172a;font-weight:700;margin:0 0 4px;">✉️ Email</p><p style="font-size:13px;color:var(--text-muted);margin:0 0 16px;">sekretariat@dprk.acehjaya.go.id</p><p style="font-size:12px;color:#0f172a;font-weight:700;margin:0 0 4px;">📍 Alamat</p><p style="font-size:13px;color:var(--text-muted);margin:0 0 16px;">Jl. Merdeka No. 01, Calang, Aceh Jaya</p></div><hr style="border:none;border-top:1px solid var(--border);margin:20px 0;"><p style="font-size:11px;color:var(--text-muted);line-height:1.6;"><strong>Jam layanan:</strong><br>Senin–Jumat: 08.00–16.00 WIB<br>Sabtu–Minggu: Libur</p></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
# =========================================================
    ...
    st.markdown('<div class="content">', unsafe_allow_html=True)
    st.markdown('<section class="section"><div class="section-kicker">Dokumentasi Hukum</div><h2 class="section-title">JDIH & Transparansi</h2><div class="section-desc">Akses daftar produk hukum dan informasi publik DPRK Aceh Jaya.</div></section>', unsafe_allow_html=True)
    
    info_cols = st.columns(4)
    cards = [("⚖️", "Produk Hukum", "Qanun dan dokumen hukum daerah."), ("📊", "Transparansi", "Informasi penyelenggaraan pemerintahan."), ("📂", "Dokumen Publik", "Dokumen yang dapat diakses masyarakat."), ("📑", "Informasi Berkala", "Informasi yang diterbitkan secara berkala.")]
    for i, (icon, title, desc) in enumerate(cards):
        with info_cols[i]:
            st.markdown(f'<div class="service-box" style="text-align:left;min-height:150px;"><div class="service-icon" style="margin:0 0 14px;">{icon}</div><div class="service-title">{title}</div><div class="service-desc">{desc}</div></div>', unsafe_allow_html=True)

    st.markdown('<div style="margin-top:56px;"><div class="section-kicker">Database</div><h2 class="section-title" style="font-size:23px;margin-bottom:20px;">Produk Hukum Daerah</h2></div>', unsafe_allow_html=True)
    df = pd.DataFrame(JDIH_DATA, columns=["No", "Nomor & Tahun", "Tentang", "Status", "Ukuran"])
    st.dataframe(df, use_container_width=True, hide_index=True, column_config={"No": st.column_config.TextColumn("No", width="50px"), "Nomor & Tahun": st.column_config.TextColumn("Nomor & Tahun", width="150px"), "Tentang": st.column_config.TextColumn("Tentang / Judul Dokumen"), "Status": st.column_config.TextColumn("Status"), "Ukuran": st.column_config.TextColumn("Unduh", help="Klik untuk mengunduh dokumen")})
    st.markdown("</div>", unsafe_allow_html=True)
...
# =========================================================
    ...
    st.markdown('<div class="content">', unsafe_allow_html=True)
    st.markdown('<section class="section"><div class="section-kicker">Informasi Kontak</div><h2 class="section-title">Hubungi DPRK Aceh Jaya</h2><div class="section-desc">Gunakan informasi berikut untuk mendapatkan layanan dan informasi dari Sekretariat DPRK Aceh Jaya.</div></section>', unsafe_allow_html=True)
    
    cols = st.columns(3)
    contacts = [("📍", "Alamat Kantor", "Jl. Merdeka No. 01, Calang, Kabupaten Aceh Jaya, Aceh 23654"), ("📞", "Telepon & Faks", "Telp: (0655) 12345\nFaks: (0655) 12346"), ("✉️", "Email Resmi", "sekretariat@dprk.acehjaya.go.id\npengaduan@dprk.acehjaya.go.id")]
    for i, (icon, title, value) in enumerate(contacts):
        with cols[i]:
            st.markdown(f'<div class="service-box" style="min-height:180px;text-align:left;"><div class="service-icon" style="margin:0 0 16px 0;">{icon}</div><div class="service-title" style="text-align:left;">{title}</div><div class="service-desc" style="font-size:13px;text-align:left;white-space: pre-line;">{value}</div></div>', unsafe_allow_html=True)

    st.markdown('<div style="margin-top:40px;background:var(--white);border:1px solid var(--border);border-radius:12px;padding:32px;"><div class="section-kicker">Sekretariat</div><h3 style="font-family:\'Plus Jakarta Sans\';color:#0f172a;font-size:20px;font-weight:700;margin-bottom:12px;">Jam Pelayanan</h3><p style="font-size:14px;color:var(--text-muted);line-height:1.8;"><strong>Senin – Kamis:</strong> 08.00 – 16.30 WIB<br><strong>Jumat:</strong> 08.00 – 16.30 WIB<br><strong>Sabtu – Minggu:</strong> Libur</p></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
<div class="premium-footer">
    <div class="footer-grid">
        <div>
            ...
            <p class="footer-description">
                Mewujudkan transparansi, akuntabilitas, dan pelayanan prima. 
                Akses informasi kelembagaan, produk hukum, dan salurkan aspirasi 
                Anda untuk pembangunan Aceh Jaya yang lebih baik.
            </p>
            <div class="footer-social">
                <div class="footer-social-item" title="Facebook">f</div>
                <div class="footer-social-item" title="Twitter / X">𝕏</div>
                <div class="footer-social-item" title="YouTube">▶</div>
                <div class="footer-social-item" title="Instagram">◎</div>
            </div>
        </div>
        ...
        <div>
            ...
            <a href="#" class="footer-link">Profil & Pimpinan</a>
            <a href="#" class="footer-link">Berita & Agenda</a>
            <a href="#" class="footer-link">Fungsi & Komisi</a>
            <a href="#" class="footer-link">PORA XV 2026</a>
        </div>

        <div>
            <h4 class="footer-heading">Layanan Publik</h4>
            <a href="#" class="footer-link">Pengaduan Masyarakat</a>
            <a href="#" class="footer-link">JDIH & Produk Hukum</a>
            <a href="#" class="footer-link">Permohonan Informasi (PPID)</a>
        ...
        <div>
            ...
            <div class="footer-contact-item">
                ...
                <a href="mailto:sekretariat@dprk.acehjaya.go.id" class="footer-link" style="margin:0; display:inline;">sekretariat@dprk.acehjaya.go.id</a>
            </div>
            <div class="footer-contact-item">
                <span class="footer-contact-icon">🕐</span>
    ...
    <div class="footer-bottom">
        <div class="footer-bottom-left">
            © 2026 Dewan Perwakilan Rakyat Kabupaten Aceh Jaya. Seluruh hak cipta dilindungi.
        </div>
        <div class="footer-bottom-right">
            Dikembangkan untuk Transparansi & Pelayanan Publik
        </div>
    </div>
</div>
""",
